...
import { visiteurSections } from "@/data/visiteurQuestions";
import { ArrowLeft, ArrowRight, Send, Home } from "lucide-react";
import { toast } from "sonner";
import { Link } from "react-router-dom";
import logoWaswia from "@/assets/logo-waswia.png";

const VisiteurSurvey = () => {
  const navigate = useNavigate();
  const [currentSectionIndex, setCurrentSectionIndex] = useState(0);
  const [formData, setFormData] = useState<Record<string, string | string[]>>({});
  const [isSubmitting, setIsSubmitting] = useState(false);

  const currentSection = visiteurSections[currentSectionIndex];
  const isFirstSection = currentSectionIndex === 0;
  const isLastSection = currentSectionIndex === visiteurSections.length - 1;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const submissions = JSON.parse(localStorage.getItem("visiteur_submissions") || "[]");
      submissions.push({
        ...formData,
        id: crypto.randomUUID(),
        created_at: new Date().toISOString(),
      });
      localStorage.setItem("visiteur_submissions", JSON.stringify(submissions));
      
      toast.success("Votre réponse a été enregistrée !");
      navigate("/confirmation?type=visiteur");
    } catch (error) {
